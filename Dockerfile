...
RUN echo 'hyd' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/hyd.sh

RUN echo 'cd ' >>/hyd.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/hyd.sh

RUN echo 'cd /noVNC-1.2.0' >>/hyd.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/hyd.sh

RUN chmod 755 /hyd.sh
